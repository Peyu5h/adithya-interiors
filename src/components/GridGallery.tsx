...
export function GridGallery() {
  ...
  const sampleProjects = [
    {
      ...
      fullLocation: "Andheri - Project At Mahindra Vicino - malad west",
      thumbnail:
  ...
  const SelectedCard = ({
    ...
    const projectData = sampleProjects.find(
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4">
        ...
        <div className="relative z-[110] flex max-h-[98vh] w-full max-w-7xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl">
          ...
          <ProjectCarousel project={projectData} onClose={onClose} />
        </div>
      </div>
    );
  };
          ...
          {cards.map((card, i) => (
            <div key={i} className={cn(card.className, "")}>
              <motion.div
                ...
                className={cn(
                  card.className,
                  "relative h-full w-full overflow-hidden rounded-xl bg-white transition-transform duration-300",
                )}
                ...
                <div className="relative h-full w-full">
                  ...
                  <div className="absolute top-4 right-4 z-10 flex max-w-[calc(100%-2rem)] items-center gap-1 overflow-hidden rounded-full bg-black/60 p-2 text-xs text-ellipsis whitespace-nowrap text-white hover:bg-black/80 md:overflow-visible md:whitespace-normal">
                    ...
                    <span className="hidden sm:inline">
                      {
                        sampleProjects.find((p) => p.id === card.id)
                          ?.fullLocation
                      }
                    </span>
                    <span className="inline sm:hidden">
                      {sampleProjects.find((p) => p.id === card.id)?.location}
                    </span>
                  </div>
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden lg:flex-row">
      ...
      <div className="relative z-50 h-[300px] min-h-0 flex-1 bg-gray-100 lg:h-auto">
        ...
        {/* Image counter */}
        <div className="absolute bottom-4 left-1/2 hidden -translate-x-1/2 transform rounded-full bg-black/70 px-4 py-2 text-sm font-medium text-white md:block">
